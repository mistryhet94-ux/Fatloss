/* ============ STORAGE HELPERS ============ */
/* Uses localStorage — data is saved in this browser only. */
const STORAGE_PREFIX = 'fatlossapp:';
...
async function loadProfile() {
  profile = await storageGet('profile', profile);
}
...
async function loadWeightLog() {
  weightLog = await storageGet('weight-log', []);
}
...
async function renderWeightView() {
  ...
  if (weightLog.length === 0) {
    ...
    [...weightLog].reverse().forEach((entry) => {
      ...
      row.querySelector('.del').onclick = async () => {
        weightLog = weightLog.filter(e => e.date !== entry.date);
        await storageSet('weight-log', weightLog);
        renderWeightView();
      };
  ...
  document.getElementById('log-weight-btn').onclick = async () => {
    ...
    weightLog = weightLog.filter(e => e.date !== today); // replace same-day entry
...
async function loadCalorieLog() {
  calorieLog = await storageGet('calorie-log', []);
}
...
async function renderCaloriesView() {
  ...
  if (calorieLog.length === 0) {
    ...
    [...calorieLog].reverse().forEach((entry) => {
      ...
      `;
      row.querySelector('.del').onclick = async () => {
        calorieLog = calorieLog.filter(e => e.date !== entry.date);
        await storageSet('calorie-log', calorieLog);
        renderCaloriesView();
      };
      logCard.appendChild(row);
    });
  ...
  document.getElementById('log-cal-btn').onclick = async () => {
    ...
    if (!val || val <= 0) return;
    calorieLog = calorieLog.filter(e => e.date !== today);
    calorieLog.push({ date: today, calories: val });
    calorieLog.sort((a, b) => a.date.localeCompare(b.date));
    await storageSet('calorie-log', calorieLog);
    renderCaloriesView();
  };
}
